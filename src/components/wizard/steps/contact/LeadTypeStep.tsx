...
interface LeadTypeStepProps {
  data: ContactFormData;
  update: (updates: Partial<ContactFormData>) => void;
}

export function LeadTypeStep({ data, update }: LeadTypeStepProps) {
  return (
    <div className="flex flex-col gap-8 h-full pr-2 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
        <TypeCard 
          ...
          active={data.leadType === "Buyer"}
          onClick={() => update({ leadType: "Buyer" })}
        />
        <TypeCard 
          title="Seller" 
          icon="sell" 
          description="Looking to list." 
          active={data.leadType === "Seller"}
          onClick={() => update({ leadType: "Seller" })}
        />
        <TypeCard 
          title="Investor" 
          icon="account_balance" 
          description="High-volume ROI focus." 
          active={data.leadType === "Investor"}
          onClick={() => update({ leadType: "Investor" })}
        />
        <TypeCard 
          title="Agent" 
          icon="badge" 
          description="Agent-to-Agent marketing." 
          active={data.leadType === "Agent"}
          onClick={() => update({ leadType: "Agent" })}
        />
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 mt-4">
        <div className="flex-1 bg-onyx-surface border border-[#27373a] rounded-xl p-6 flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            {["ASAP", "Soon", "Later"].map((t) => (
              <button 
                key={t}
                onClick={() => update({ timeline: t })}
                className={`flex-1 py-3 rounded-lg border transition-all ${
                  data.timeline === t 
                    ? "border-cyan bg-cyan/10 text-cyan font-bold shadow-[0_0_10px_rgba(0,209,255,0.15)]" 
                    : "border-[#27373a] text-slate-400 hover:border-cyan hover:text-cyan"
                }`}
              >
                {t}
              </button>
        ...
        <div className="flex-1 bg-onyx-surface border border-cyan/30 rounded-xl p-6 flex flex-col gap-4 shadow-[0_0_20px_rgba(0,209,255,0.05)]">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-cyan/10 text-cyan px-2 py-1 rounded-md border border-cyan/20 font-black uppercase tracking-widest">Optional</span>
          </div>
          <label className="flex items-start gap-4 cursor-pointer group">
            <div className="mt-1 relative">
              <input 
                ...
                checked={data.a2pOptIn}
                onChange={(e) => update({ a2pOptIn: e.target.checked })}
              />
            ...
            <div className="flex-1">
              <span className="text-slate-200 font-bold block mb-1">Required to Send Text</span>
...
function TypeCard({ title, icon, description, active, onClick }: { title: string, icon: string, description: string, active?: boolean, onClick: () => void }) {
  return (
    <div 
      onClick={onClick}
      className={`p-6 rounded-xl border-2 transition-all cursor-pointer flex flex-col gap-4 ${active ? 'bg-cyan/5 border-cyan shadow-[0_0_15px_rgba(0,209,255,0.2)] scale-[1.02]' : 'bg-onyx-surface border-[#27373a] hover:border-slate-500 hover:scale-[1.02]'}`}
    >
